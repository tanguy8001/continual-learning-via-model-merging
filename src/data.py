...
def double_loaders(dataset, path, batch_size, num_workers, transform_name, digit=None, use_test=False,
            shuffle_train=True, cifar_class="dog"):
    # Determine the dataset class from torchvision
    ds = getattr(datasets, dataset)
    path = os.path.join(path, dataset.lower())
    transform = getattr(getattr(Transforms, dataset), transform_name)
    ...
    if dataset.lower() == "mnist":
        ...
        print("Loaded CIFAR10.")
        train_set = datasets.CIFAR10(root=path, download=True, train=True, transform=transform.train)
        test_set = datasets.CIFAR10(root=path, train=False, download=True, transform=transform.test)

        if cifar_class is not None:
            print(f'Using the heterogeneous data-split setup with class {cifar_class}.')
            
            # Separate indices for the cifar_class to exclude and other classes
            indices_class = [i for i, (x, y) in enumerate(train_set) if y == cifar_class]
            other_classes_indices = [i for i, (x, y) in enumerate(train_set) if y != cifar_class]

            # Split data for models A and B
            # Model A gets 10% of data for class 'cifar_class'
            indices_10_percent = other_classes_indices[:int(0.1 * len(other_classes_indices))]

            # Model B gets 90% of the training data for remaining classes
            indices_90_percent = other_classes_indices[int(0.1 * len(other_classes_indices)):]

            # Create data subsets
            dataset_A = Subset(train_set, indices_10_percent)
            dataset_A += Subset(train_set, indices_class)
            dataset_B = Subset(train_set, indices_90_percent)
