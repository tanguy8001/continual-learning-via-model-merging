...
class TLPFusionRNN:
    ...
    def __init__(self, args, base_models, target_model, data):
        ...
        # Helps to maintain a global copy of the regularization
        self.tlp_sinkhorn_reg = self.args.tlp_sinkhorn_regularization
    ...
    def fuse(self):
        ...
        for i in range(1, self.target_model.num_layers + 1):
            ...
            pis_model_1.append(cur_pi[0]) # model 1
        ...
        permuted_model_1_path = path.join(self.args.save_path, 'permuted_model_1.pth')
        ...
        permuted_model_2_path = path.join(self.args.save_path, 'permuted_model_2.pth')
    ...
    def generate_permuted_model(self, model, pis, path):
        ...
        for i in range(1, len(pis)):
            ...
            input_dim = model.channels[i-1]
            hid_dim = model.channels[i]

            if self.args.model_name in ['LSTM', 'lstm'] and i != len(pis) - 1:
                Ws = Ws.reshape(4, hid_dim, input_dim)
                Hs = Hs.reshape(4, hid_dim, hid_dim)

            # Compute permuted Ws
            Ws_permuted = torch.matmul(cur_pi, torch.matmul(Ws, k_l * k_l_prev * prev_pi.transpose(0, 1)))
            if i != len(pis) - 1: # if not last layer
                Ws_permuted = Ws_permuted.view(-1, input_dim)
            permuted_model_weights.append(Ws_permuted)
            
            # Compute permuted Hs
            if Hs is not None:
                Hs_permuted = torch.matmul(cur_pi, torch.matmul(Hs, k_l * k_l * cur_pi.transpose(0, 1)))
                Hs_permuted = Hs_permuted.view(-1, hid_dim) 
                permuted_model_weights.append(Hs_permuted)

        for parameter, weight in zip(permuted_model.parameters(), permuted_model_weights):
    ...
    def fuse_single_layer(self, layer, prev_pis):
        ...
        for model in self.base_models:
            ...
            if self.args.model_name in ['LSTM', 'lstm'] and layer != model.num_layers:
                input_dim = model.channels[layer - 1]
                hid_dim = model.channels[layer]
                Ws = Ws.reshape(4, hid_dim, input_dim) # reshape the weights of hidden layer of LSTM
                Hs = Hs.reshape(4, hid_dim, hid_dim)
            base_Ws.append(Ws)
            base_Hs.append(Hs)
        ...
        if layer == 1 and self.args.encoder:# Input Embedding layer, the weight shoudl be transposed
        ...
        elif layer == self.target_model.num_layers: # output linear layer (decoder)
            return self.fuse_single_linear_layer_helper(len(self.base_models), layer, layer == self.target_model.num_layers,
                                             base_Ws, target_Ws,prev_pis)
        else: # hidden layer
    ...
    def fuse_single_hidden_layer_helper(self, num_models, layer, is_last_layer, base_Ws,base_Hs, target_Ws,target_Hs, prev_pis):
        ...
        if self.args.model_name in ['LSTM', 'lstm']:
            logging.info('Reshaping the hidden layer of LSTM.')
            input_dim = self.target_model.channels[layer - 1]
            hid_dim = self.target_model.channels[layer]
            Ws = Ws.reshape(4, hid_dim, input_dim)  # return 4 concatenated weights
            Hs = Hs.reshape(4, hid_dim, hid_dim)
        ...
        for i in range(1, max_iter + 1):
            ...
            # new_pi = self.solve_pi_hidden(base_Ws, base_Hs, target_Ws, target_Hs, prev_pis, pis, layer == 1)
            ...
            for pi in pis:
                logging.info("Pi min: {}, Pi max: {}, Pi sum.{}".format(
                    pi.min().detach().cpu(), pi.max().detach().cpu(),
                    pi.sum().detach().cpu()))
            #obj = self.compute_barcenter_objective(base_Ws, base_Hs, target_Ws, target_Hs, prev_pis, pis, layer == 1)
        ...
        if self.args.model_name in ['LSTM', 'lstm']:
            Ws = Ws.reshape(-1, input_dim)
            Hs = Hs.reshape(-1, hid_dim)
        target_Ws.data = Ws.data
        target_Hs.data = Hs.data
    ...
    def solve_pi_hidden(self, base_Ws, base_Hs, target_W, target_H, prev_pis, cur_pis,
        ...
        # print('shape of target_Ws:', target_W.size())
        # print('shape of target_Hs:', target_H.size())
        error = 1.0
        threshold = 1e-5
        num_iters = 0
        max_iters = 100
        while error >= threshold and num_iters < max_iters:
            new_pis = self.solve_pi_hidden_helper(base_Ws, base_Hs, target_W, target_H,
                                                  prev_pis, cur_pis, is_first_layer)
            error = 0.0
            for new_pi_i, cur_pi_i in zip(new_pis, cur_pis):
                error += (new_pi_i - cur_pi_i).pow(2).mean().detach().cpu()
            cur_pis = new_pis
            num_iters += 1
            ...
            if is_first_layer:
                ...
                for W_i, H_i,cur_pi_i in zip(base_Ws, base_Hs, cur_pis):
                    ...
                    try:
                        ...
                        # cost = (W_t - W_i).pow(2).sum(-1) / (1 + self.alpha_h) + self.alpha_h * ((H_t - H_i).pow(2) * cur_pi_i).sum(-1).sum(-1) / (1 + self.alpha_h)
                        if len(cost.size()) == 3:
                            cost = cost.sum(0)
                        ...
                        if error.startswith("CUDA out of memory."):
                            ...
                            if len(H_t.size()) == 5: # LSTM
                                ...
                                for i in range(4):
                                    # print('shape of H_t:', i,  H_t[i].size(), H_i[i].size())
                                ...
                                # cost = (W_t - W_i).pow(2).sum(-1) / (1 + self.alpha_h) + self.alpha_h * memory_safe_methods.get_cost(H_t, H_i, cur_pi_i) / (1 + self.alpha_h)
                        else:
                            print(error)
                            raise ImportError(e)
                ...
                for W_i, H_i, prev_pi_i, cur_pi_i in zip(base_Ws, base_Hs, prev_pis, cur_pis):
                    ...
                    try:
                        ...
                        # cost = ((W_t - W_i).pow(2) * prev_pi_i).sum(-1).sum(-1) / (1 + self.alpha_h) + self.alpha_h * ((H_t - H_i).pow(2) * cur_pi_i).sum(-1).sum(-1) / (1 + self.alpha_h)
                        if len(cost.size()) == 3:
                            cost = cost.sum(0)
                        ...
                        if error.startswith("CUDA out of memory."):
                            if len(H_t.size()) == 4:
                                ...
                                # cost = self.alpha_h * memory_safe_methods.get_cost(H_t, H_i, cur_pi_i) / (1 + self.alpha_h) + memory_safe_methods.get_cost(W_t, W_i, prev_pi_i) / (1 + self.alpha_h)
                                ...
                                for idx_mat in range(4):
                                    ...
                                    # cost += self.alpha_h * memory_safe_methods.get_cost(H_t[idx_mat], H_i[idx_mat], cur_pi_i) / (1 + self.alpha_h)
                                    # cost += memory_safe_methods.get_cost(W_t[idx_mat], W_i[idx_mat], prev_pi_i) / (1 + self.alpha_h)
                            else:
