...
sys.path.append('../')
import logging
from rnn_dataset_torch import ShakeSpeareCharDataset
from tlp_rnn_fusion.rnn_models import RNNWithEncoderDecoder,LSTMWithEncoderDecoder #RNNModel
...
def get_dataloaders(exp_args):
    if exp_args.dataset_name == "shakespeare_char":
        train_dataset = ShakeSpeareCharDataset(exp_args.train_data_path_x,exp_args.train_data_path_y)
        test_dataset = ShakeSpeareCharDataset(exp_args.test_data_path_x,exp_args.test_data_path_y)
    else:
...
def main():
    ...
    Set up model
    """
    model_path = exp_args.model_load_path
    ...
    # all_hid_dims = np.arange(128,384+1,16)
    assert len(regularizations) == len(model_files),"regs"+str(len(regularizations))+',models'+str(len(model_files))
    # assert len(all_hid_dims) == len(model_files),"regs"+str(len(all_hid_dims))+',models'+str(len(model_files))
    for i in range(len(model_files)):
        print("current file being evaluated:",model_files[i])
        exp_args.model_load_path = model_path + model_files[i]
        # hidden_dims = [all_hid_dims[i]]
        if exp_args.model_name == "rnn":
            # dimensions of input, hidden, output must be specified
            model = RNNWithEncoderDecoder(output_dim = exp_args.vocab_size, embed_dim=exp_args.embed_dim,hidden_dims=hidden_dims, hidden_activations=hidden_activations,bias= exp_args.bias)
        elif exp_args.model_name == "lstm":
            # dimensions of input, hidden, output must be specified
            model = LSTMWithEncoderDecoder(input_dim = exp_args.vocab_size, embed_dim=exp_args.embed_dim,hidden_dims=hidden_dims, hidden_activations=hidden_activations,bias= exp_args.bias)
        ...
        if exp_args.evaluate:
            ...
            all_test_acc.append(test_acc.detach().cpu())
        else:
            trainer.train()
        
    
    np.savetxt(model_path+"all_test_acc.csv",all_test_acc)
