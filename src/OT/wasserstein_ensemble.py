...
import torch
import numpy as np
import OT.routines as routines
from OT.model import get_model_from_name
import OT.utils as utils
from OT.ground_metric import GroundMetric
import math
...
def get_wassersteinized_layers_modularized(args, networks, activations=None, eps=1e-7, test_loader=None):
    ...
    # Iterate through each layer of the two networks
...
def geometric_ensembling_modularized(args, networks, test_loader, activations=None):
    ...
    #else:
        #raise("not supported")
    # For now, we don't support activation mode for continual learning
