...
'''
Helper fct to filter dataset by specific labels 'label_ids'.
Returns: new DataLoader with filtered dataset
'''
...
'''
Split MNIST into two parts based on a choice of labels.
'''
def split_mnist_by_labels(args, train_loader, test_loader, choice=None):
    ...
    other = np.setdiff1d(total, choice) # Returns unique values in 'total' but not in 'choice'
